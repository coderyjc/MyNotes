...
> 来源：黑马程序员 - bilibili - [HDFS](https://www.bilibili.com/video/BV1154y1U73k)
...
![1561706903642](R:\GITHUB\MyNotes\_Typora\BigData\DistributedFileSystem\Hadoop-HDFS.imgs\1561706903642.png)		
...
![1561706945153](R:\GITHUB\MyNotes\_Typora\BigData\DistributedFileSystem\Hadoop-HDFS.imgs\1561706945153.png)	
...
这个block 是逻辑单位，事实上如果一个文件是130M，将会切分为两个block，一个是128M，另一个是2M。

即：block的大小是不确定的，最大位是128M。
...
![1561706988677](R:\GITHUB\MyNotes\_Typora\BigData\DistributedFileSystem\Hadoop-HDFS.imgs\1561706988677.png)	
...
~~~shell
格式   hdfs dfs  -get [-ignorecrc ]  [-crc]  <src> <localdst>作用：将文件拷贝到本地文件系统。 CRC 校验失败的文件通过-ignorecrc选项拷贝。 文件和CRC校验和可以通过-CRC选项拷贝
~~~
...
~~~shell
格式  ： hdfs  dfs -mv URI   <dest>作用： 将hdfs上的文件从原路径移动到目标路径（移动之后文件删除），该命令不能夸文件系统
~~~
...
## 9.HDFS 文件写入过程
 ...
 ![](R:\GITHUB\MyNotes\_Typora\BigData\DistributedFileSystem\Hadoop-HDFS.imgs\1.png)
...
![1558574532408](R:\GITHUB\MyNotes\_Typora\BigData\DistributedFileSystem\Hadoop-HDFS.imgs\1558574532408.png)	
...
```xml
<property>    <name>dfs.namenode.name.dir</name>        <value>        file:///export/servers/hadoop2.7.5/hadoopDatas/namenodeDatas,          	        		file:///export/servers/hadoop-2.7.5/hadoopDatas/namenodeDatas2    </value></property><property>     <name>dfs.namenode.edits.dir</name>     <value>file:///export/servers/hadoop-2.7.5/hadoopDatas/nn/edits</value</property>>
```
...
```shell
cd /export/servers/hadoop2.7.5/hadoopDatas/namenodeDatashdfs oiv -i fsimage_0000000000000000864 -p XML -o hello.xml
```
...
```shell
cd /export/servers/hadoop2.7.5/hadoopDatas/namenodeDatashdfs oev -i  edits_0000000000000000865-0000000000000000866 -p XML -o myedit.xml 
```
...
* 配置 SecondaryNameNode
  ...
  * 修改 `core-site.xml`, 这一步不做配置保持默认也可以
    ...
    ![1561707242581](R:\GITHUB\MyNotes\_Typora\BigData\DistributedFileSystem\Hadoop-HDFS.imgs\1561707242581.png)		
