...
- 可以在查找的时候加上`-i` 表示不区分大小写

缺点：

- 如果新建了一个文件，他是没有被收录到文件库中的， locate不会实时更新（可以手动更新）`updatedb` 然后再locate进行查找。
- 如果文件存在tmp中，是查找不到的，这个问题无解。
...
功能：搜索命令所目录**及别名**相关的信息

语法：`which [命令]`

背景：我们已经知道了一般来说在bin目录下的命令是所有用户都可以使用的，而在sbin或者/usr/sbin下的命令是只有root才能使用的，如何知道这个命令在哪个目录下面？用which。比如 `which locate` 
...
功能：搜索命令所目录**及其帮助文档**的信息（绝大多数的命令的帮助文档都存放在usr/share/man/下面）
...
功能：在文件中搜索字符串匹配的行进行输出

语法：`grep -iv [排除指定字符串] [文件]`
...
- 在查看配置文件的时候千万不要写绝对路径，只写`man services` 就行了。

- 了解两种帮助的类型: 1：命令的帮助，5:配置文件的帮助

如果只想获得命令的相关信息或者配置文件的相关信息，则可以使用：`whatis [命令]` 或者 `apropos [配置文件]` 进行查看

如果只想知道命令的使用方式，则可以在命令后面加上--help，语法：`[命令] --help`
...
功能描述：获得Shell内置命令（用which或者whereis找不到命令所在的路径）的帮助信息
...
- 普通用户更改密码的时候必须符合复杂度（8位以上大小写字母+数字）的要求，但是root不用。
- 普通用户只能改自己的密码，root能改任何人的密码
...
功能描述：给用户发消息，以`Ctrl + D`结束，前提是所有的用户都**已经登录在了在服务器**上
...
语法：`lastlog`  查看所有用户最后登录的时间

- `-u [UId]` 查看某特定用户上次登录的时间
...
功能：显示数据包到主机间的路径，并返回数据包在每个节点之间的时间
...
关机命令后面的数字是0，重启命令后面的数字是6，这两个数字是什么意思，有什么联系吗？

- 表示系统的运行级别
- 0 关机；1 单用户；2 不完全多用户，不含DFS服务；3 完全多用户；4 未分配；5 图形界面；6 重启
...
<img src="Linux.assets\image-20201003085550162.png" alt="image-20201003085550162" style="zoom:80%;" />
...
- 越是对**安全性**要求越高的服务器越需要建立合理的用户权限等级制度和服务器操作规范。
...
> 在文件中虽然有密码标志，但是里面存放的不是真正的密码（是一个'x'），这个x表示“这个用户是有密码的”，如果连这个x都没有，那么系统就会认为这个用户没有密码。真正的密码存放在了shadow文件中，而且已经被加密过了。
...
> 初始组和附加组</br>
> 初始组：用户*一登陆*就立刻拥有这个用户组的相关权限，每个用户的初始组只能有*一个*，一般就是和这个用户的用户名相同的组名作为这个用户的初始组。</br>
> 附加组：用户可以加入多个其他的用户组并拥有这些组的权限，附加组可以*有多个*。</br>
> 虽然初始组能改，但是并不推荐更改初始组。
...
- 第 6 字段：家目录
    ...
    - 超级用户：/root/（所有者和所属组都是root用户，权限是 550）

> 就算把一个用户的初始组ID变为了0组，这个用户的权限也不会改变。</br> 只有把用户的UID改成 0 之后， 这个用户才能行驶root权限。

- 第 7 字段：登录之后的shell

> Shell 是什么？</br> 简单来说就是Linux的命令解释器
...
> 加密算法是 SHA512 散列加密算法 </br> 如果密码是‘！！’ 或者 “ * ” 表示没有密码，不能登录

- 第 3 字段：密码最后一次修改日期

> 使用 1970.1.1 作为基准时间，每过去一天时间戳 + 1

- 第 4 字段：两次密码的修改间隔时间（和第 3 字段相比）

- 第 5 字段：密码有效期（和第 3 字段相比）

- 第 6 字段：密码修改到期前的警告天数，在这个事件内，每次登录的时候都会提醒你修改密码（第 5 字段相比）

- 第 7 字段：密码过去之后的宽限天数（和第 5 字段相比）
    - 0：代表密码过期后立即失效
    - -1：代表密码永远不会失效
...
> 时间戳换算命令： `date -d "1970-01-01 16076 days"`
...
- `-u [UID]` 指定用户的UID号
- `-d [家目录]` 指定新用户的家目录(不推荐)
- `-c [用户说明]` 指定用户的说明(文字中有空格要加上双引号)
- `-g [组名]` 指定用户的初始组
- `-G [组名]` 指定用户的附加组，如果要加到多个组中，要在多个组中间用逗号分开。
